...
> 为 Claude Code 设计的**企业级远程数据分析解决方案**，通过小浣熊 API 提供专业的数据分析、可视化和代码执行能力。

## ✨ 核心特性

- 🔄 **双接口架构** - 智能路由办公解释器和数据分析接口
- 📊 **专业可视化** - 雷达图、柱状图、散点图、热力图等
- 📁 **多格式支持** - Excel、CSV、图片、文本文件处理
- ⚡ **流式交互** - 实时显示分析过程和代码执行
...
| 场景 | 接口选择 | 输出位置 | 适用场景 |
|------|----------|----------|----------|
| **有文件** | 办公解释器API | `./raccoon/dataanalysis/` | Excel分析、图片处理 |
| **纯计算** | 数据分析API | `./raccoon/dataanalysis/` | 算法实现、数据生成 |
...
```bash
# 查看活跃会话
python3 scripts/main.py sessions --list

# 清理旧会话
python3 scripts/main.py sessions --delete SESSION_ID
```
...
| 类型 | 格式 | 用途 | 最大大小 |
|------|------|------|----------|
| **表格** | `.xlsx`, `.xls`, `.csv` | 数据分析、统计建模 | 50MB |
| **图片** | `.png`, `.jpg`, `.jpeg` | OCR识别、图像分析 | 20MB |
| **文本** | `.txt`, `.md`, `.json` | 文本挖掘、格式转换 | 10MB |
...
python3 scripts/main.py analyze \
  ...
  --prompt "分批处理数据，每次分析1000行，生成分段报告"

# 复杂分析 - 分步执行
python3 scripts/main.py analyze \
  --prompt "步骤1：数据清洗和预处理"
python3 scripts/main.py analyze \
  --prompt "步骤2：特征工程和模型训练"
```
...
这是一个**生产级别的数据分析代理系统**，适用于：
- 🏢 **企业BI团队** - 快速数据洞察和报告生成
- 🎓 **学术研究** - 统计分析和科学计算
- 📊 **数据科学** - 原型开发和算法验证
- 💹 **金融分析** - 风险建模和投资决策

**⚠️ 重要提醒**: 使用前请确保已正确配置API凭证和网络环境。所有数据处理均在远程安全沙盒中完成。